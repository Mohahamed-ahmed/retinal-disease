...
function Summary1({ PatientAnswers }) {
  const skippedAnswers = PatientAnswers.filter((answer) => answer === null);
  const NumberOfQuestion = images.length;
  console.log(NumberOfQuestion);
  const correctAnswers = PatientAnswers.filter(
    (answer, index) => answer === images[index].answers[0]
  );
    ...
    <div id="summary">
      ...
      <ol>
        {PatientAnswers.map((answer, index) => {
          ...
          if (answer === null) {
            cssClass += " skipped";
          } else if (answer === images[index].answers[0]) {
            cssClass += " correct";
          } else {
            cssClass += " wrong";
          }
          ...
          return (
            <li key={index}>
              <h3>{index + 1}</h3>
              <p className="question">{images[index].text}</p>
